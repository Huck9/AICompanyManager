...
if (isset($_GET['id'])) {
    if (!empty($_POST)) {
        ...
        $stmt = $pdo->prepare("Update salesInvoice set invoiceNumber = ?, contractorName = ?, vatID = ?, nettoValue =?, vatValue =?, bruttoValue=?, nettoValueOther = ?, nettoOtherName =?, filename=?, date=? where id = ?");

...
if (isset($_GET['id'])) {
    if (!empty($_POST)) {
        ...
        $stmt = $pdo->prepare("Update purchaseinvoice set invoiceNumber = ?, contractorName = ?, vatID = ?, nettoValue =?, vatValue =?, bruttoValue=?, nettoValueOther = ?, nettoOtherName =?, filename=? where id = ?");
